...
URL_DEMUXER=$( (cat $VERSIONS_FILE | grep -Eo '"demuxerUrl"[^,]*' | grep -Eo '[^:]*$') | sed -e 's/^.//' -e 's/.$//' -e 's/^.//')
URL_CONNECTD=$( (cat $VERSIONS_FILE | grep -Eo '"connectdUrl"[^,]*' | grep -Eo '[^:]*$') | sed -e 's/^.//' -e 's/.$//' -e 's/^.//')

CLI=$( (cat $VERSIONS_FILE | grep -Eo '"cli"[^,]*' | grep -Eo '[^:]*$') | sed -e 's/^.//' -e 's/.$//' -e 's/^.//')
MUXER=$( (cat $VERSIONS_FILE | grep -Eo '"muxer"[^,]*' | grep -Eo '[^:]*$') | sed -e 's/^.//' -e 's/.$//' -e 's/^.//')
DEMUXER=$( (cat $VERSIONS_FILE | grep -Eo '"demuxer"[^,]*' | grep -Eo '[^:]*$') | sed -e 's/^.//' -e 's/.$//' -e 's/^.//')
CONNECTD=$( (cat $VERSIONS_FILE | grep -Eo '"connectd"[^,]*' | grep -Eo '[^:]*$') | sed -e 's/^.//' -e 's/.$//' -e 's/^.//')
...
if [ "$(uname)" = "Darwin" ]; then
    ...
    #amd64
    mkdir -pm 777 ./bin/x64/
    curl -L https://${URL_CLI}${CLI}/remoteit.x86_64-linux --output ./bin/x64/remoteit
    curl -L https://${URL_CONNECTD}${CONNECTD}/connectd.x86_64-etch --output ./bin/x64/connectd
    curl -L https://${URL_DEMUXER}${DEMUXER}/demuxer.x86_64-etch --output ./bin/x64/demuxer
    curl -L https://${URL_MUXER}${MUXER}/muxer.x86_64-etch --output ./bin/x64/muxer
